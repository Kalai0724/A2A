...
"""
"""IMPORTANT: When searching for hotels, always use the following SQL format:
SELECT name, city, hotel_type, room_type, price_per_night FROM hotels WHERE city = '[CITY]' AND hotel_type = '[HOTEL_TYPE]' AND room_type = '[ROOM_TYPE]'

If no results for the requested hotel_type, try the next available type in this order: HOTEL, AIRBNB, PRIVATE_PROPERTY.
If no results for the requested room_type, try the next available type in this order: STANDARD, SUITE, SINGLE, DOUBLE.
Always confirm with the user before booking an alternative."""
...
"""
"""IMPORTANT: When searching for rental cars, always use the following SQL format:
SELECT provider, city, type_of_car, daily_rate FROM rental_cars WHERE city = '[CITY]' AND type_of_car = '[CAR_TYPE]'

If no results for the requested car_type, try the next available type in this order: TRUCK, SUV, SEDAN.
Always confirm with the user before booking an alternative."""

...
def normalize_date(date_str):
    try:
        ...
        return date_str  # fallback if already correct
...
class TravelAgent(BaseAgent):
    ...
    async def stream(
        ...
        def normalize_dates_in_text(text):
            # Replace dates like 'January 20, 2026' with '2026-01-20'
            import re
        ...
        try:
            ...
            # Normalize all date strings in the query before passing to the agent
            if isinstance(query, str):
                query = normalize_dates_in_text(query)

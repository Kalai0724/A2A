...
def normalize_flights(amadeus_json):
    """
    Convert Amadeus flight offers to a simple list.
    """
    flights = []
    offers = amadeus_json.get("data", [])
    ...
    for offer in offers[:8]:
        ...
        flights.append({
            "airline": (offer.get("validatingAirlineCodes") or ["N/A"])[0],
            "departure": first_seg["departure"]["at"],
            "arrival": last_seg["arrival"]["at"],
            "stops": max(len(itineraries[0]["segments"]) - 1, 0),
    ...
    try:
        ...
        data = await amadeus.get(
            ...
            params={
                ...
                "currencyCode": "USD",
                "max": 8,
                # Add returnDate only if present
